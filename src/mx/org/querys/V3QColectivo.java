...
"        PERIODO \n" +
"        from "+Con1+"TR_colectivo"+Con2+" ) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO) WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' and periodo = '"+PValidacion.periodo+"'  \n" +
...
public ArrayList Año_Expe_ColectivoNE(String Con1, String Con2){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT * FROM(\n" +
        "SELECT * FROM(\n" +
        ...
        "PERIODO\n" +
        "from "+Con1+"TR_colectivo"+Con2+" ) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO AND EXPE_AÑO NOT IN ('2021','2022','2023','2020','2024','2025')) WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' and periodo = '"+PValidacion.periodo+"' \n" +
        "or clave_organo='"+PValidacion.clave_organo+"' and periodo = '"+PValidacion.periodo+"'";
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
    ...
    public ArrayList ColectivoFechaAperturaMenor2020(String Con1, String Con2){
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT \n" +
"    CLAVE_ORGANO,\n" +
...
"TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM "+Con1+"TR_COLECTIVO"+Con2+" \n" +
"WHERE \n" +
...
public ArrayList FECHA_RESOLUCION_AJ_FUT(String Con1, String Con2){
      conexion.Conectar();
      Array = new ArrayList();
...
public ArrayList Duplicidad_expediente(String Con1, String Con2){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,TO_CHAR(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY')FECHA_APERTURA_EXPEDIENTE,TO_NUMBER(regexp_replace(expediente_clave, '[^0-9]', ''))expediente_clave2,PERIODO FROM "+Con1+"TR_COLECTIVO"+Con2+"\n" +
...
"select CLAVE_ORGANO,expediente_clave,TO_NUMBER(regexp_replace(expediente_clave, '[^0-9]', ''))expediente_clave2,PERIODO from "+Con1+"TR_COLECTIVO"+Con2+" \n" +
...
//Query de validacion donde la fecha de admisionde la demanda no debe de ser menor a la fecha de apertura del expediente
    public ArrayList Fecha_Admi_DemandaNE(String Con1, String Con2){
      ...
      sql="select * from (select entidad_clave,clave_organo,expediente_clave,FECHA_APERTURA_EXPEDIENTE,fecha_admi_demanda,periodo\n" +
          "from "+Con1+"TR_colectivo"+Con2+" where  periodo = '"+PValidacion.periodo+"'\n" +
...
"FROM "+Con1+"TR_AUDIENCIAS"+Con2+" P,"+Con1+"TR_COLECTIVO"+Con2+" S\n" +
