 ...
 */
public class TR_FED_COLECTIVO {

    public int IN_FED_COLECTIVO() throws Exception {
        ...
        try {
            ...
            ArrayList<BeanTR_FED_COLECTIVO> ad = new ArrayList<>();
            ArrayList<ArrayList<String>> fila = QFed.TMP_TO_TR_FED_COLECTIVO();
            if (fila != null) {
                for (int i = 0; i < fila.size(); i++) {
                    BeanTR_FED_COLECTIVO c = new BeanTR_FED_COLECTIVO();
                    ...
                    for (int j = 0; j < 37; j++) {
                        ...
                        c.PREG_TRAMITACION_DEPURACION(filaActual.get(25));
                        c.FECHA_DEPURACION(filaActual.get(26));
                        ...
                        c.COD_ORG(filaActual.get(35));
                        c.COD_EXPE(filaActual.get(36));
                    }
                    ad.add(c);
                    CFilas++;
                }
                if (CFilas > 0) {
                    sd = StructDescriptor.createDescriptor("OBJ_TR_FED_COLECTIVO", con);
                    ...
                    descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_TR_FED_COLECTIVO", con);
                    System.out.println("entro 3");
                    array_to_pass = new ARRAY(descriptor, con, structs);
                    System.out.println("entro 4");
                    st = con.prepareCall("{? = call(PKG_INTEGRADOR_TR.Colectivo(?))}");
                    ...
                    JOptionPane.showMessageDialog(null, "TR_FED_COLECTIVO sin registros");
                }
            }
            return CFilas;
        } catch (Exception ex) {
