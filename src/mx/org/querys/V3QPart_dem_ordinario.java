...
"from(\n" +
"select * from(\n" +
"select * from "+Con1+"tr_ordinario"+Con2+" where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
" CANTIDAD_DEMANDADOS>0) where expediente_clave not in (select expediente_clave from "+Con1+"TR_part_dem_ordinario"+Con2+" where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"AND PERIODO ='"+PValidacion.periodo+"'\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE,PERIODO)PRIN LEFT JOIN "+Con1+"TR_ORDINARIO"+Con2+" SEC\n" +
