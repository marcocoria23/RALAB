...
public ArrayList ExpeNDesglose(String Con1, String Con2){
  conexion.Conectar();
    Array = new ArrayList();
...
"from(\n" +
"select * from(\n" +
"select * from "+Con1+"tr_colectivo"+Con2+" where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
") where expediente_clave not in (select expediente_clave from "+Con1+"tr_part_ACT_colectivo"+Con2+" where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
public ArrayList IncompetenciaNE(String Con1, String Con2){
    conexion.Conectar();
    Array = new ArrayList();
   /* sql="select * from(\n" +
"select * from(\n" +
...
"group by clave_organo,expediente_clave) prin  right join "+Con1+"tr_part_ACT_colectivo"+Con2+" sec\n" +
...
"group by sec.clave_organo,sec.expediente_clave,prin.desglose_actores) prin  right join  "+Con1+"TR_COLECTIVO"+Con2+" sec\n" +
"on  prin.clave_organo=sec.clave_organo and  prin.expediente_clave=sec.expediente_clave\n" +
...
"where incompetencia='Sí'";*/
     sql="SELECT * FROM(\n" +
"SELECT PRIN.CLAVE_ORGANO,PRIN.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_PART,SEC.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_COLECTIVO,decode(sec.incompetencia,'1','Sí','2','NO')incompetencia FROM(\n" +
    ...
    System.out.println(sql);
      try {
          while (resul.next()) {
              Array.add(new String[]{
                  resul.getString("CLAVE_ORGANO"),
                  resul.getString("EXPEDIENTE_CLAVE_COLECTIVO"),
                  resul.getString("INCOMPETENCIA")
                });
          }
      conexion.close();
...
public ArrayList Estatus_demandaNE(String Con1, String Con2){
    conexion.Conectar();
    Array = new ArrayList();
     sql="SELECT * FROM(\n" +
"SELECT PRIN.CLAVE_ORGANO,PRIN.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_PART,SEC.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_COLECTIVO,decode(sec.estatus_demanda,'1','Admitida','2','Desechada','3','Archivo','4','No se dio trámite al escrito de demanda') estatus_demanda FROM(\n" +
"SELECT UNIQUE(EXPEDIENTE_CLAVE)EXPEDIENTE_CLAVE,CLAVE_ORGANO,PERIODO\n" +
"FROM "+Con1+"tr_part_ACT_colectivo"+Con2+"\n" +
"WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' AND PERIODO='"+PValidacion.periodo+"'\n" +
"OR CLAVE_ORGANO='"+PValidacion.clave_organo+"' AND PERIODO='"+PValidacion.periodo+"')PRIN LEFT JOIN\n" +
...
public ArrayList Dif_ActoresNE(String Con1, String Con2){
    conexion.Conectar();
    Array = new ArrayList();
    sql="SELECT * FROM(\n" +
...
"AND PERIODO ='"+PValidacion.periodo+"'\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE,PERIODO)PRIN LEFT JOIN "+Con1+"TR_COLECTIVO"+Con2+" SEC\n" +
