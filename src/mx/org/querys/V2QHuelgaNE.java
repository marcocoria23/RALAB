...
"        PERIODO \n" +
"        from V2_TR_huelgajl ) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO) WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' and periodo = '"+PValidacion.periodo+"'  \n" +
...
"WHERE to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy') < to_date(FECHA_PRESENTA_PETIC,'dd/mm/yyyy')) where  periodo = '"+PValidacion.periodo+"'\n" +
          ...
          "and FECHA_PRESENTA_PETIC <> '09/09/1899'  and periodo = '"+PValidacion.periodo+"'  and periodo = '"+PValidacion.periodo+"'\n";
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
          while (resul.next()) {
...
public ArrayList Fecha_EmplazamientoNE(){
      ...
      sql="select * from (select entidad_clave,clave_organo,expediente_clave,to_char(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY') FECHA_APERTURA_EXPEDIENTE,to_char(FECHA_EMPLAZAMIENTO,'DD/MM/YYYY') FECHA_EMPLAZAMIENTO  ,periodo\n" +
          "from V2_TR_huelgajl where  periodo = '"+PValidacion.periodo+"'\n" +
...
// LA FECHA DE AUDIENCIA CELEBRARA NO DEBE SER MENOR A LA FECHA DE PRESENTACION DE LA DEMANDA
  public ArrayList Fecha_Aud_Presentacion(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (SELECT S.ENTIDAD_CLAVE,P.CLAVE_ORGANO,P.EXPEDIENTE_CLAVE,to_char(P.FECHA_AUDIEN_CELEBRADA,'DD/MM/YYYY')FECHA_AUDIEN_CELEBRADA,to_char(S.FECHA_PRESENTA_PETIC,'DD/MM/YYYY') FECHA_PRESENTA_PETIC, P.TIPO_PROCED,P.PERIODO\n" +
"FROM V2_TR_AUDIENCIASJL P,V2_TR_HUELGAJL S\n" +
