...
"from(\n" +
"select * from(\n" +
"select * from "+Con1+"tr_huelga"+Con2+" where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
"AND PERIODO ='"+PValidacion.periodo+"' and incompetencia=2  OR CLAVE_ORGANO='"+PValidacion.clave_organo+"' \n" +
"AND PERIODO ='"+PValidacion.periodo+"' and incompetencia=2) where \n" +
" CANTIDAD_DEMANDADOS>0) where expediente_clave not in (select expediente_clave from "+Con1+"TR_part_dem_HUELGA"+Con2+" where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE,PERIODO)PRIN LEFT JOIN "+Con1+"TR_HUELGA"+Con2+" SEC\n" +
