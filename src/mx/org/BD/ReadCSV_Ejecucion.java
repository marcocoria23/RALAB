...
public class ReadCSV_Ejecucion {
    ...
    public void Read_Ejecucion() throws FileNotFoundException, IOException {
         ...
         try {          
            ...
            if (bytesRead >= 3 && bytes[0] == (byte) 0xEF && bytes[1] == (byte) 0xBB && bytes[2] == (byte) 0xBF) {
                System.out.println("Archivo en UTF-8");
                 ...
                 IN_EJECUCION(rutaCarpetaArchivos,Insert.Periodo);
            }else{
                 System.out.println("6.....");
                conUTF8.Convertir_utf8_EBaseDatos(rutaCarpetaArchivos);
                 IN_EJECUCION(conUTF8.rutaNuevoArchivo,Insert.Periodo);
    ...
    public void IN_EJECUCION(String Ruta, String Periodo) throws Exception {
                ...
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(rutaArchivoCSV), StandardCharsets.UTF_8));
                    ...
                    if (numeroColumnas == 10) {
                        ...
                        for (CSVRecord record : csvParser) {
                            ...
                            c.SetMOTIVO_SOLICITUD_EJ(record.get(4).toUpperCase());
                            c.SetFECHA_PRESENTACION(record.get(5).toUpperCase());
                            c.SetESTATUS_EXPE(record.get(6).toUpperCase());
                            c.SetFECHA_CONCLUSION(record.get(7).toUpperCase());
                            c.SetFASE_CONCLUSION(record.get(8).toUpperCase());
                        ...
                        if (TotalRegistros > 0) {
                            ...
                            sd = StructDescriptor.createDescriptor("OBJ_EJECUCION", con);
                            ...
                            descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_EJECUCION", con);
                            System.out.println("entro 4");
                            array_to_pass = new ARRAY(descriptor, con, structs);
                            System.out.println("entro 5");
                            st = con.prepareCall("{? = call(PKG_INTEGRADORCSV.Ejecucion(?))}");
