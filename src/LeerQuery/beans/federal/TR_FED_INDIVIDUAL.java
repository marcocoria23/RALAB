 ...
 */
public class TR_FED_INDIVIDUAL {

    public int IN_FED_INDIVIDUAL() throws Exception {
        ...
        try {
            ...
            ArrayList<BeanTR_FED_INDIVIDUAL> ad = new ArrayList<>();
            ArrayList<ArrayList<String>> fila = QFed.TMP_TO_TR_FED_INDIVIDUAL();
            if (fila != null) {
                for (int i = 0; i < fila.size(); i++) {
                    BeanTR_FED_INDIVIDUAL c = new BeanTR_FED_INDIVIDUAL();
                    ...
                    for (int j = 0; j < 40; j++) {
                        ...
                        c.PREG_TRAMITACION_DEPURACION(filaActual.get(26));
                        c.FECHA_DEPURACION(filaActual.get(27));
                        c.PREG_AUD_PRELIM_CELEBR(filaActual.get(28));
                        c.FECHA_AUDIENCIA_PRELIM(filaActual.get(29));
                        c.PREG_CELEBRA_AUD_JUICIO(filaActual.get(30));
                        c.FECHA_AUD_JUICIO(filaActual.get(31));
                        c.ID_ESTATUS_EXPED(filaActual.get(32));
                        c.FECHA_ULT_ACT_PROC(filaActual.get(33));
                        c.ID_FASE_SOL_EXPED(filaActual.get(34));
                        c.ID_FORMA_SOLUCION(filaActual.get(35));
                        c.FECHA_DICTO_SOLUCION(filaActual.get(36));
                        c.ID_SENTIDO_SENTEN(filaActual.get(37));
                        c.COD_ORG(filaActual.get(38));
                        c.COD_EXPE(filaActual.get(39));
                    }
                    ad.add(c);
                    CFilas++;
                }
                if (CFilas > 0) {
                    sd = StructDescriptor.createDescriptor("OBJ_TR_FED_INDIVIDUAL", con);
                    ...
                    descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_TR_FED_INDIVIDUAL", con);
                    System.out.println("entro 3");
                    array_to_pass = new ARRAY(descriptor, con, structs);
                    System.out.println("entro 4");
                    st = con.prepareCall("{? = call(PKG_INTEGRADOR_TR.Individual(?))}");
                    ...
                    JOptionPane.showMessageDialog(null, "TR_FED_INDIVIDUAL sin registros");
                }
            }
            return CFilas;
        } catch (Exception ex) {
