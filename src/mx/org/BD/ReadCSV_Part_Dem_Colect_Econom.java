...
import Bean_Procedures.Part_Dem_Colect_Econom;
 ...
 */
public class ReadCSV_Part_Dem_Colect_Econom {
    ...
    public void Read_Part_Dem_Colect_Econom() throws FileNotFoundException, IOException {
       ...
       // FileInputStream f = new FileInputStream(Insert.rutaT);
        ...
        try {          
                ...
                if (Insert.CarpetaArchivos==true){
                 ...
                 rutaCarpetaArchivos=Insert.rutaT+"CSV_BD_T.5.3_part_dem_colec_nat_eco.csv";
            ...
            if (bytesRead >= 3 && bytes[0] == (byte) 0xEF && bytes[1] == (byte) 0xBB && bytes[2] == (byte) 0xBF) {
                System.out.println("Archivo en UTF-8");
                 ...
                 IN_PART_DEM_COLECT_ECONOM(rutaCarpetaArchivos,Insert.Periodo);
            }else{
                 System.out.println("6.....");
                conUTF8.Convertir_utf8_EBaseDatos(rutaCarpetaArchivos);
                 IN_PART_DEM_COLECT_ECONOM(conUTF8.rutaNuevoArchivo,Insert.Periodo);
    ...
    public void IN_PART_DEM_COLECT_ECONOM(String Ruta, String Periodo) throws Exception {
                ...
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(rutaArchivoCSV), StandardCharsets.UTF_8));
                    ...
                    if (numeroColumnas == 32) {
                        ...
                        for (CSVRecord record : csvParser) {
                            ...
                            TotalRegistros++;
                            Part_Dem_Colect_Econom c = new Part_Dem_Colect_Econom();
                            ...
                            c.SetDEMANDADO(record.get(4).toUpperCase());
                            c.SetDEFENSA_DEM(record.get(5).toUpperCase());
                            c.SetNOMBRE_SINDICATO_DEM(record.get(6).toUpperCase());
                            c.SetREG_ASOC_SINDICAL_DEM(record.get(7).toUpperCase());
                            c.SetTIPO_SINDICATO_DEM(record.get(8).toUpperCase());
                            c.SetOTRO_ESP_SINDICATO_DEM(record.get(9).toUpperCase());
                            c.SetORG_OBRERA_DEM(record.get(10).toUpperCase());
                            c.SetNOMBRE_ORG_OBRERA_DEM(record.get(11).toUpperCase());
                            ...
                            c.SetTIPO_DEM_PAT(record.get(17).toUpperCase());
                            ...
                            ad.add(c);
                            if (record.get(4).equals("3")) {
                                JOptionPane.showMessageDialog(null, "Error en el campo DEMANDADO fuera de catalogo  Clave_organo:" + record.get(1) + " Expediente:" + record.get(2) + " Id_demandado:" + record.get(3) + " nota: campo DEMANDADO solo puede tener opcion 1,2,4,5 Y 9");
                                Inserta = false;
                            }
                        }
                        ...
                        if (TotalRegistros > 0) {
                            if (Inserta == true) {
                                ...
                                sd = StructDescriptor.createDescriptor("OBJ_PART_DEM_COLECT_ECONOM", con);
                                ...
                                descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_PART_DEM_COLECT_ECONOM", con);
                                System.out.println("entro 4");
                                array_to_pass = new ARRAY(descriptor, con, structs);
                                System.out.println("entro 5");
                                st = con.prepareCall("{? = call(PKG_INTEGRADORCSV.Part_Dem_Colect_Econom(?))}");
    ...
    JOptionPane.showMessageDialog(
        null,
             "Tabla: V3_TMP_PART_DEM_DOLECT_ECONOMJL\n" +
