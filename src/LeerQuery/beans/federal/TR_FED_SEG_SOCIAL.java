 ...
 */
public class TR_FED_SEG_SOCIAL {

    public int IN_FED_SEG_SOCIAL() throws Exception {
        ...
        try {
            ...
            ArrayList<BeanTR_FED_SEG_SOCIAL> ad = new ArrayList<>();
            ArrayList<ArrayList<String>> fila = QFed.TMP_TO_TR_FED_SEG_SOCIAL();
            if (fila != null) {
                for (int i = 0; i < fila.size(); i++) {
                    BeanTR_FED_SEG_SOCIAL c = new BeanTR_FED_SEG_SOCIAL();
                    ...
                    for (int j = 0; j < 37; j++) {
                        ...
                        c.PREG_AUD_PRELIM_CELEBR(filaActual.get(25));
                        c.FECHA_AUDIENCIA_PRELIM(filaActual.get(26));
                        c.PREG_CELEBRA_AUD_JUICIO(filaActual.get(27));
                        c.FECHA_AUD_JUICIO(filaActual.get(28));
                        c.ID_ESTATUS_EXPED(filaActual.get(29));
                        c.FECHA_ULT_ACT_PROC(filaActual.get(30));
                        c.ID_FASE_SOL_EXPED(filaActual.get(31));
                        c.ID_FORMA_SOLUCION(filaActual.get(32));
                        c.FECHA_DICTO_SOLUCION(filaActual.get(33));
                        c.ID_SENTIDO_SENTEN(filaActual.get(34));
                        c.COD_EXPE(filaActual.get(35));
                        c.COD_ORG(filaActual.get(36));
                    }
                    ad.add(c);
                    CFilas++;
                }
                if (CFilas > 0) {
                    sd = StructDescriptor.createDescriptor("OBJ_TR_FED_SEG_SOCIAL", con);
                    ...
                    descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_TR_FED_SEG_SOCIAL", con);
                    System.out.println("entro 3");
                    array_to_pass = new ARRAY(descriptor, con, structs);
                    System.out.println("entro 4");
                    st = con.prepareCall("{? = call(PKG_INTEGRADOR_TR.Seg_social(?))}");
                    ...
                    JOptionPane.showMessageDialog(null, "TR_FED_SEG_SOCIAL sin registros");
                }
            }
            return CFilas;
        } catch (Exception ex) {
