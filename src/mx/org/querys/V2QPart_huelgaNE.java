...
public ArrayList ExpeNDesglose(){
  conexion.Conectar();
    Array = new ArrayList();
    sql="select CLAVE_ORGANO, EXPEDIENTE_CLAVE, decode(INCOMPETENCIA,2,'NO') INCOMPETENCIA, \n" +
"CANTIDAD_ACTORES,CANTIDAD_DEMANDADOS\n" +
"from(\n" +
"select * from(\n" +
"select * from v2_tr_huelgajl where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"or CANTIDAD_DEMANDADOS>0) where expediente_clave not in (select expediente_clave from v2_tr_part_huelgajl where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
public ArrayList IncompetenciaNE(){
    ...
    /*sql="select * from(\n" +
"select * from(\n" +
"select sec.clave_organo,sec.expediente_clave,DECODE(sec.incompetencia,'1','Sí')incompetencia,sec.cantidad_actores,prin.Desglose_actores,sec.cantidad_demandados\n" +
...
"group by sec.clave_organo,sec.expediente_clave,prin.desglose_actores) prin  right join  V2_TR_HUELGAjl sec\n" +
"on  prin.clave_organo=sec.clave_organo and  prin.expediente_clave=sec.expediente_clave\n" +
...
+ "PRIN.PERIODO,SEC.FASE_SOLI_EXPEDIENTE  \n" +
"FROM(\n" +
...
"AND PERIODO ='"+PValidacion.periodo+"'\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE,PERIODO)PRIN LEFT JOIN V2_TR_HUELGAJL SEC\n" +
