...
ResultSet resul;
 ...
 //Query de validacion donde el año de la fecha de apertura es diferente al año de la clave del expediente.
    public ArrayList Año_Expe_Ejecucion(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT * FROM(\n" +
        "SELECT * FROM(\n" +
        ...
        "PERIODO\n" +
        "from V1_TR_ejecucionjl ) WHERE  FECHA_APERTURA <>EXPE_AÑO) WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' and periodo = '"+PValidacion.periodo+"' \n" +
...
public ArrayList Fecha_Conclusion(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (select ID,clave_organo,expediente_clave,to_char(fecha_apertura,'DD/MM/YYYY') fecha_apertura,to_char(fecha_conclusion,'DD/MM/YYYY') fecha_conclusion  ,periodo\n" +
          "from V1_TR_EJECUCIONjl where periodo = '"+PValidacion.periodo+"'\n" +
          ...
          "and periodo = '"+PValidacion.periodo+"' ) where  to_date(fecha_conclusion,'dd/mm/yyyy')  < to_date(fecha_apertura,'dd/mm/yyyy')";
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
          while (resul.next()) {
