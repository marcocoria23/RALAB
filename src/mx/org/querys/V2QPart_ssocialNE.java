 ...
 */
public class V2QPart_ssocialNE {
...
"from(\n" +
"select * from(\n" +
"select * from v2_tr_seg_socialJL where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"or CANTIDAD_DEMANDADOS>0) where expediente_clave not in (select expediente_clave from v2_tr_part_seg_socialJL where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"from V2_TR_part_seg_socialjl\n" +
"where substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"' and periodo='"+PValidacion.periodo+"'\n" +
"and id_actor not like '%-%' \n" +
"group by clave_organo,expediente_clave) prin  right join V2_TR_part_seg_socialjl sec\n" +
...
"group by sec.clave_organo,sec.expediente_clave,prin.desglose_actores) prin  right join  V2_TR_seg_socialjl sec\n" +
"on  prin.clave_organo=sec.clave_organo and  prin.expediente_clave=sec.expediente_clave\n" +
...
"where incompetencia='Sí'";*/
  sql="SELECT * FROM(\n" +
"SELECT PRIN.CLAVE_ORGANO,PRIN.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_PART,SEC.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_seg_social,decode(sec.incompetencia,'1','Sí','2','NO')incompetencia FROM(\n" +
    ...
    System.out.println(sql);
      try {
          while (resul.next()) {
              Array.add(new String[]{
                  resul.getString("CLAVE_ORGANO"),
                  resul.getString("EXPEDIENTE_CLAVE_seg_social"),
                  resul.getString("INCOMPETENCIA")
                });
          }
      conexion.close();
...
"where ESTATUS_DEMANDA IN ('Desechada','Archivo','No se dio trámite al escrito de demanda')";*/
    sql="SELECT * FROM(\n" +
"SELECT PRIN.CLAVE_ORGANO,PRIN.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_PART,SEC.EXPEDIENTE_CLAVE EXPEDIENTE_CLAVE_seg_social,decode(sec.estatus_demanda,'1','Admitida','2','Desechada','3','Archivo','4','No se dio trámite al escrito de demanda') estatus_demanda FROM(\n" +
"SELECT UNIQUE(EXPEDIENTE_CLAVE)EXPEDIENTE_CLAVE,CLAVE_ORGANO,PERIODO\n" +
"FROM V2_TR_PART_seg_socialJL\n" +
"WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' AND PERIODO='"+PValidacion.periodo+"'\n" +
"OR CLAVE_ORGANO='"+PValidacion.clave_organo+"' AND PERIODO='"+PValidacion.periodo+"')PRIN LEFT JOIN\n" +
...
"AND PERIODO ='"+PValidacion.periodo+"'\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE,PERIODO)PRIN LEFT JOIN V2_TR_seg_socialjl SEC\n" +
