...
import Bean_Procedures.Tercerias;
 ...
 */
public class ReadCSV_Tercerias {
    ...
    public void Read_Tercerias() throws FileNotFoundException, IOException {
       ...
       // FileInputStream f = new FileInputStream(Insert.rutaT);
         ...
         try {          
                ...
                if (Insert.CarpetaArchivos==true){
                 ...
                 rutaCarpetaArchivos=Insert.rutaT+"CSV_BD_T.7.1_terce.csv";
            ...
            if (bytesRead >= 3 && bytes[0] == (byte) 0xEF && bytes[1] == (byte) 0xBB && bytes[2] == (byte) 0xBF) {
                System.out.println("Archivo en UTF-8");
                 ...
                 IN_TERCERIAS(rutaCarpetaArchivos,Insert.Periodo);
            }else{
                 System.out.println("6.....");
                conUTF8.Convertir_utf8_EBaseDatos(rutaCarpetaArchivos);
                 IN_TERCERIAS(conUTF8.rutaNuevoArchivo,Insert.Periodo);
    ...
    public void IN_TERCERIAS(String Ruta, String Periodo) throws Exception {
                ...
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(rutaArchivoCSV), StandardCharsets.UTF_8));
                    ...
                    if (numeroColumnas == 13) {
                        ...
                        for (CSVRecord record : csvParser) {
                            ...
                            c.SetCLAVE_INCIDENTAL(record.get(3).toUpperCase());
                            c.SetFECHA_INCIDENTE(record.get(4).toUpperCase());
                            c.SetFECHA_APERTURA_INCIDENTAL(record.get(5).toUpperCase());
                            c.SetTIPO_INCIDENTE(record.get(6).toUpperCase());
                            c.SetCELEBRACION_AUDIENCIA(record.get(7).toUpperCase());
                            c.SetFECHA_AUDIENCIA(record.get(8).toUpperCase());
                            c.SetESTATUS_EXPEDIENTE(record.get(9).toUpperCase());
                            c.SetSENTENCIA_INCIDENTAL(record.get(10).toUpperCase());
                            c.SetFECHA_RESOLUCION(record.get(11).toUpperCase());
                            c.SetCOMENTARIOS(record.get(12).toUpperCase());
                        ...
                        if (TotalRegistros > 0) {
                            ...
                            sd = StructDescriptor.createDescriptor("OBJ_TERCERIAS", con);
                            ...
                            descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_TERCERIAS", con);
                            System.out.println("entro 4");
                            array_to_pass = new ARRAY(descriptor, con, structs);
                            System.out.println("entro 5");
                            st = con.prepareCall("{? = call(PKG_INTEGRADORCSV.Tercerias(?))}");
