...
import Bean_Procedures.Part_Dem_Individual;
 ...
 */
public class ReadCSV_Part_Dem_Individual {
    ...
    public void Read_Part_Dem_Individual() throws FileNotFoundException, IOException {
      ...
      //  FileInputStream f = new FileInputStream(Insert.rutaT);
        ...
        try {          
                ...
                if (Insert.CarpetaArchivos==true){
                 ...
                 rutaCarpetaArchivos=Insert.rutaT+"CSV_BD_T.2.3_part_dem_esp_indiv.csv";
            ...
            if (bytesRead >= 3 && bytes[0] == (byte) 0xEF && bytes[1] == (byte) 0xBB && bytes[2] == (byte) 0xBF) {
                System.out.println("Archivo en UTF-8");
                 ...
                 IN_PART_DEM_INDIVIDUAL(rutaCarpetaArchivos,Insert.Periodo);
            }else{
                 System.out.println("6.....");
                conUTF8.Convertir_utf8_EBaseDatos(rutaCarpetaArchivos);
                 IN_PART_DEM_INDIVIDUAL(conUTF8.rutaNuevoArchivo,Insert.Periodo);
    ...
    public void IN_PART_DEM_INDIVIDUAL(String Ruta, String Periodo) throws Exception {
            ...
            if (bytesRead >= 3 && bytes[0] == (byte) 0xEF && bytes[1] == (byte) 0xBB && bytes[2] == (byte) 0xBF) {
                ...
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(rutaArchivoCSV), StandardCharsets.UTF_8));
                    ...
                    if (numeroColumnas == 22) {
                        ...
                        ArrayList<Part_Dem_Individual> ad = new ArrayList<>();
                        for (CSVRecord record : csvParser) {
                            //System.out.println("llenado de csv");
                            TotalRegistros++;
                            Part_Dem_Individual c = new Part_Dem_Individual();
                            ...
                            c.SetTIPO(record.get(6).toUpperCase());
                            ...
                            ad.add(c);
                            if (record.get(4).equals("2") || record.get(4).equals("3") || record.get(4).equals("4")) {
                                JOptionPane.showMessageDialog(null, "Error en el campo DEMANDADO fuera de catalogo  Clave_organo:" + record.get(1) + " Expediente:" + record.get(2) + "Id_demandado:" + record.get(3) + " nota: campo DEMANDADO solo puede tener opcion 1,5 Y 9");
                                Inserta = false;
                            }
                        }
                        ...
                        if (TotalRegistros > 0) {
                            if (Inserta == true) {
                                ...
                                sd = StructDescriptor.createDescriptor("OBJ_PART_DEM_INDIVIDUAL", con);
                                ...
                                descriptor = ArrayDescriptor.createDescriptor("ARR_OBJ_PART_DEM_INDIVIDUAL", con);
                                System.out.println("entro 4");
                                array_to_pass = new ARRAY(descriptor, con, structs);
                                System.out.println("entro 5");
                                st = con.prepareCall("{? = call(PKG_INTEGRADORCSV.Part_Dem_Individual(?))}");
